use crate::{ast::expr::{Expr, ExprType, Literal}, token::{Token, TokenType}, types::Type, value::Value};

use super::runtime_error::RuntimeError::{self, *};
...
impl ValueInterpreter for Expr {
    fn interpret(&self) -> Result<Value> {
...
impl ValueInterpreter for Literal {
    fn interpret(&self) -> Result<Value> {
        Ok(match self {
